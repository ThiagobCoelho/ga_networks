...
if __name__ == "__main__":
	...
	net_wave, net_adj, net_time, net_links, num_channels = net.generate(top)
	...
	with open('block_GA_%d.m' % num_channels, 'a') as f:
		text = str(blocked_ga).replace('[','').replace(']','')
		f.write('%s; ...\n' % text)
	
	with open('block_ALT_%d.m' % num_channels, 'a') as f:
		text = str(blocked_alt).replace('[','').replace(']','')
		f.write('%s; ...\n' % text)
	
	with open('block_FIX_%d.m' % num_channels, 'a') as f:
